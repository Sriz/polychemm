...
for ($i = 0; $i < count($consumptions); $i++):
    $html .= "<td>".$consumptions[$i]['TblMixingIssue']['nepalidate']."</td>";
endfor;
$html .="</tr><tr>
<td><strong>Shift</strong></td>";
    for ($i = 0; $i < count($consumptions); $i++):
        $html.="<td>".$consumptions[$i]['TblMixingIssue']['shift']."</td>";
    endfor;
$html .="</tr>";
$html .="<tr>
<td><strong>Material</strong></td>";
    for ($i = 0; $i < count($consumptions); $i++):
    $html .="<td>".$consumptions[$i]['TblMixingIssue']['material']."</td>";
    endfor;
$html .="</tr>";
foreach ($material_lists as $m):
    $html .="<tr>
        <td><strong>".$m['MixingPattern']['pattern_name']."</strong></td>";
        for ($i = 0; $i < count($consumptions); $i++):
            $materials = json_decode($consumptions[$i]['TblMixingIssue']['patterns']);
            $material = isset($materials->$m['MixingPattern']['id']) ? ($materials->$m['MixingPattern']['id'] ? $materials->$m['MixingPattern']['id']:0):0;
            $html .="<td>".$material ."</td>";
        endfor;
    $html .= "</tr>";
endforeach;
